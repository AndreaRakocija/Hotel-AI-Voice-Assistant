...
async function fetchOpenAIAnswer(messages) {
  const recentMessages = messages.slice(-10); // only last 10 for context
  const res = await fetch("/api/chat", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export default function HotelAIVoiceDemo() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "assistant",
      text: "Hello — I'm your Hotel AI Assistant. Ask me about rooms, packages, dining, or say 'talk to a human'.",
    },
  ]);
  const [status, setStatus] = useState("ready");
  const [callbackLog, setCallbackLog] = useState([]);
  ...
  async function submitUserMessage(text) {
    ...
    if (/\b(human|person|operator|representative|agent|reps)\b/i.test(text)) {
      simulateStaffNotification(text);
      const reply =
          "I’ve notified our staff. They will follow up by phone or email shortly.";
      setMessages([...updatedMessages, { role: "assistant", text: reply }]);
      speak(reply);
      return;
    }
    ...
    // fallback
    const fallback =
        "I don’t have that info right now. Try asking about rooms, packages, dining, check-in, or say 'talk to a human'.";
    setMessages((prev) => [...prev, { role: "assistant", text: fallback }]);
    speak(fallback);
  }
  ...
  return (
      <div className="min-h-screen bg-gradient-to-b from-slate-100 to-slate-200 p-6 flex flex-col items-center">
        <div className="w-full max-w-3xl bg-white rounded-3xl shadow-lg flex flex-col overflow-hidden">
          {/* Header */}
          <header className="flex items-center justify-between p-4 bg-gradient-to-r from-sky-500 to-blue-500 text-white rounded-t-3xl">
            <h1 className="text-xl md:text-2xl font-bold">
              Hotel AI Assistant — Voice Demo
            </h1>
          ...
          <main className="flex flex-col md:flex-row gap-6 p-4 flex-1 overflow-hidden">
            ...
            <section className="flex-1 flex flex-col h-[500px] md:h-[600px]">
              {/* Chat Messages */}
              <div
                  className="flex-1 overflow-y-auto p-4 space-y-3 bg-slate-50 rounded-xl shadow-inner"
                  id="chat-box"
              >
                {messages.map((m, i) => (
                    <div
              ...
              <div className="mt-3 flex gap-3">
                <input
                    ...
                    className="w-full py-2 px-3 rounded-2xl shadow-lg text-lg focus:outline-none focus:ring-4 focus:ring-sky-400 focus:border-sky-400 transition-all"
                    value={transcript}
                ...
                <button
                    ...
                    className="px-5 py-3 bg-sky-600 text-white font-semibold rounded-xl shadow-md hover:bg-sky-700"
                >
                  Send
                </button>
              </div>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {samples.map((s) => (
                    <button
                        key={s}
                        onClick={() => submitUserMessage(s)}
                        className="px-3 py-1 bg-white border rounded-full shadow-sm hover:bg-sky-50 text-sm"
                    >
                      {s}
                    </button>
                ))}
